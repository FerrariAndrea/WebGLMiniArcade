class Car{
    ...
    constructor(obj_car,obj_wheel,x=0,y=0,z=0,o=0){
            ...
            this.fari_a_dest=[0.0,-2.0,-5.0];

            this.posRel_faro_ad={};
            this.posRel_faro_ad.x=0.664 -0.2;
            this.posRel_faro_ad.y=0.666;
            this.posRel_faro_ad.z=-1.663 +1.0;

            this.posRel_faro_as={};
            this.posRel_faro_as.x=-(0.664 -0.2);
            this.posRel_faro_as.y=0.666;
            this.posRel_faro_as.z=-1.663 +1.0;
    ...
    draw(gl,programInfo){
        ...
        var faro_dst = transformVector3(tempM_fari,this.fari_a_dest);
        ...
        faro_dst = transformVector3(tempM_fari,this.fari_a_dest);
      ...
      enableKeyboard(enable){
       ...
       doKeyDown(e){
        ...
        drawFrustum(gl,cameraMatrix,colorProgramInfo,projectionMatrix,cubeLinesBufferInfo){
            ...
            //--------------------------FARO DESTRA
            const lightProjectionMatrix_d = this.perspective
            ? m4.perspective(
            ...
            const viewMatrix_d = m4.inverse(cameraMatrix);            
            ...
            // infinity
            const mat_d = m4.multiply(this.lightWorldMatrix.ad, m4.inverse(lightProjectionMatrix_d));        
            ...
            webglUtils.drawBufferInfo(gl, cubeLinesBufferInfo, gl.LINES);
             //--------------------------FARO SINISTRA
             const lightProjectionMatrix_s = this.perspective
             ? m4.perspective(
                 degToRad(this.fieldOfView_A),
                 this.projWidth / this.projHeight,
                 0.5,  // near
             ...
             const viewMatrix_s = m4.inverse(cameraMatrix);            
             gl.useProgram(colorProgramInfo.program);                  
             // Setup all the needed attributes.
             webglUtils.setBuffersAndAttributes(gl, colorProgramInfo, cubeLinesBufferInfo);                  
             // scale the cube in Z so it's really long
             // to represent the texture is being projected to
             // infinity
             const mat_s = m4.multiply(this.lightWorldMatrix.as, m4.inverse(lightProjectionMatrix_s));        
             // Set the uniforms we just computed
             webglUtils.setUniforms(colorProgramInfo, {
               u_color: [0, 0, 1, 1],
               u_view: viewMatrix_s,
