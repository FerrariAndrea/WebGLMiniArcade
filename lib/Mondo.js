...
class Mondo{
    ...
    getBound(){
        //per il mondo i bound sono invertiti (sono validi se l'auto è al suo interno)
        //la funzione restituita è semplicemente in base alla grandezza del mondo come segue
        const grandezzaMondo = this.sizeMondo/2;
        return function(x,y){
                //true se sei dentro al mondo
               return( Math.abs(x)<grandezzaMondo && Math.abs(y)<grandezzaMondo)                
        }
    }
}