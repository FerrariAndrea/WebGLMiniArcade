...
class Pulsantiera{ 
      ...
      if(s>-1 && s< this.testiAttuali.length){
        ...
        if(this.testiAttuali[s]==="Cambia visuale: libera"){
          ...
          console.log(this.gp.settings.radius);
            ...
            GamePanel.settings.theta-=degToRad(this.offsetRotazione); 
        }else if(this.testiAttuali[s]==="Ruota a sinistra"){
            GamePanel.settings.theta+=degToRad(this.offsetRotazione);
        }else if(this.testiAttuali[s]==="Ruota in alto"){
            GamePanel.settings.phi-= degToRad(this.offsetRotazione);  
             //limiti visuale mobile
             if(radToDeg(GamePanel.settings.phi)<=0){
                GamePanel.settings.phi= degToRad(0.1);  
            }
        }else if(this.testiAttuali[s]==="Ruota in basso"){
            GamePanel.settings.phi+= degToRad(this.offsetRotazione); 
             //limiti visuale mobile
            if(radToDeg(GamePanel.settings.phi)>=90){
                GamePanel.settings.phi= degToRad(89);  
            }
        }
    ...
    mouseMove(event){
        ...
        const offsetTop = getreferedSize(this.getCanvasH(),10);
        const marginTop = 5;
        const w= getreferedSize(this.getCanvasW(),86);
        const h=getreferedSize(this.getCanvasH(),88/count)-marginTop;
        for(var i=0;i<count;i++){
            //controllo se il cursore risiede nel quadrato del tasto
            if(x>marginLeft && x< marginLeft+w && y>i*(marginTop+h)+offsetTop && y< i*(marginTop+h)+h+offsetTop){
...
class Panel2D{
    ...
    constructor(canvas,GamePanel){                           
        ...
        this.pulsante = "red";        
        this.pulsante_selected = "blue";
        //-------------------------------
        this.fpscounter = 0;
        //pulsantiera
        this.pulsantiera = new Pulsantiera(canvas,GamePanel);
    ...
    drawPulsante(index,testo,n){
        ...
        const marginTop = 5;
        const w= getreferedSize(this.getCanvasW(),86);
        const h=getreferedSize(this.getCanvasH(),88/n)-marginTop;
    ...
    render(){      
        ...
        //     console.log(text_size)            
        //     console.log( getreferedSize(this.getCanvasH(),10))
        this.ctx.fillText(str,2, 20,text_size);
        this.ctx.stroke();
        //pulsantiera
