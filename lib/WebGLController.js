class WebGLControllerV2{
        ...
        drawScene(projectionMatrix,cameraMatrix,textureMatrix,lightWorldMatrix,programInfo,mMatrix=null) {
              ...
              //per ogni obj
              const uniforms = {
                u_colorMult: [0.5, 1, 1, 1],  // lightblue
                u_color: [1, 0, 0, 1],
                u_world: this.getObjects()[0].mMatrix,
              };
                const objBufferInfo=this.getObjects()[0].getBufferInfo(gl);
                if(mMatrix!==null){//inutile
                        //console.log("this.getObjects()[0].mMatrix)",this.getObjects()[0].mMatrix);
                        gl.uniformMatrix4fv(mMatrix, false, this.getObjects()[0].mMatrix);
                }
                ...
                // ---------------------------------------------------------------- Draw the plane --------
                //gl.uniformMatrix4fv(mMatrix, false, m4.identity());
                const planeUniforms = {
                        u_colorMult: [1, 0.5, 1, 1],  // lightblue
                        u_color: [1, 0, 0, 1],
                        u_texture: this.checkerboardTexture,
                        u_world: m4.translation(-10, -10, -10),
                      };
        ...
        render(){
                ...
                // {
                  ...
                  // Setup all the needed attributes.
                  const objBufferInfo=this.getObjects()[0].getBufferInfo(gl);
                  webglUtils.setBuffersAndAttributes(gl, this.colorProgramInfo, objBufferInfo);
                  ...
                  // calls gl.drawArrays or gl.drawElements
                  webglUtils.drawBufferInfo(gl, objBufferInfo, gl.LINES);
                // }
